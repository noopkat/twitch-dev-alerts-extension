...
/* 
 PREACT APP 
*/
const Placeholder = () => (<li><img src={`${assetsDir}/images/coffee.svg`} height="15" />No activity yet!</li>);

const GithubEvent = (props) => { 
  const { event, event: { html_url }, flash } = props; 

  return (
    <li class={flash ? 'animate' : ''}>
      <img src={`${assetsDir}/images/${getEventIcon(event)}`} height="15"/> 
...
class GithubEventList extends Component {
  ...
  render(props, state) {
    ...
    const list = latestEvents.map((event, index) => {
      return <GithubEvent flash={index === 0 ? flash : false} event={event} />
    });
    ...
    return (
      <div>
        <div id='header'>
          ...
          <button id='hideButton' onClick={() => this.toggleVisibility()} title={state.visible ? 'hide' : 'show'}>
            <img src={`${assetsDir}/images/${state.visible ? 'eye-off' : 'eye'}.svg`} height='13'/>
          </button>
        </div>  
